...
public class AlgoritmoGenetico {
    ...
    public AlgoritmoGenetico(int tamanhoPopulacao, double capacidadeMochila, int probabilidadeMutacao, int qtdeCruzamentos) {
    ...
    public void executar() {
        ...
        int indiceMenor = encontrarMenorIndividuo();
        System.out.println("\nIndivíduo de menor avaliação: Cromossomo " + indiceMenor + "\n");
    ...
    public int encontrarMenorIndividuo() {
        double menorFitness = Double.MAX_VALUE;
        int indiceMenorFitness = -1;

        for (int i = 0; i < this.tamPopulacao; i++) {
            ArrayList<Integer> cromossomo = populacao.get(i);
            double avaliacao = fitness(cromossomo);

            if (avaliacao < menorFitness) {
                menorFitness = avaliacao;
                indiceMenorFitness = i;
            }
        }

        return indiceMenorFitness;
    }
}